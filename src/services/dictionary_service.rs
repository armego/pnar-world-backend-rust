use crate::{
    dto::{
        ...
        responses::{DictionaryEntryResponse, PaginatedResponse}, 
        SearchDictionaryRequest, UpdateDictionaryEntryRequest
    },
    error::AppError,
};
use sqlx::{PgPool, Row};
...
pub async fn create_entry(
    ...
    let entry_record = sqlx::query(
        ...
        INSERT INTO pnar_dictionary (
            ...
            example_pnar, example_english, created_by, created_at, updated_at
        )
...
pub async fn get_entry(
    ...
    let entry_record = entry_record.ok_or_else(|| AppError::NotFound("Dictionary entry not found".to_string()))?;
...
pub async fn list_entries(
    ...
    page: i64,
    per_page: i64,
) -> Result<PaginatedResponse<DictionaryEntryResponse>, AppError> {
...
pub async fn update_entry(
    ...
    let existing = existing.ok_or_else(|| AppError::NotFound("Dictionary entry not found".to_string()))?;
    let created_by: Option<Uuid> = existing.get("created_by");

    if created_by != Some(user_id) {
        return Err(AppError::Forbidden("You can only update your own entries".to_string()));
    }
    ...
    let entry_record = sqlx::query(
        ...
        SET 
            ...
            example_pnar = COALESCE($5, example_pnar),
            example_english = COALESCE($6, example_english),
            updated_at = NOW()
        WHERE id = $1
        RETURNING id, pnar_word, english_word, part_of_speech, definition,
                  example_pnar, example_english, difficulty_level, usage_frequency,
...
pub async fn delete_entry(
    ...
    if created_by != Some(user_id) {
        return Err(AppError::Forbidden("You can only delete your own entries".to_string()));
    }
